  ...
  return (
    <header className="sticky top-0 z-40 border-b border-[color:var(--color-ink-100)] bg-[color:var(--color-surface)]/90 backdrop-blur">
      <div className="mx-auto flex w-full max-w-6xl items-center justify-between px-4 py-4 md:px-6">
        ...
        >
          <span className="h-9 w-9 rounded-full border border-[color:var(--color-ink-200)] bg-[color:var(--color-primary)] shadow-sm" />
         Apple Tv
        </NavLink>
        ...
        <nav className="hidden items-center gap-6 md:flex">
          ...
          <NavLink
            to="/submit"
            className="rounded-full border border-[color:var(--color-ink-200)] px-4 py-2 text-xs font-semibold uppercase tracking-[0.2em] text-[color:var(--color-ink-800)] transition-colors hover:border-[color:var(--color-primary)] hover:text-[color:var(--color-primary)]"
          >
            Send News
          </NavLink>
        </nav>
      ...
      {open ? (
        <div className="border-t border-[color:var(--color-ink-100)] bg-[color:var(--color-surface)] md:hidden">
          <nav className="mx-auto flex w-full max-w-6xl flex-col gap-4 px-4 py-6">
            ...
            <NavLink
              to="/submit"
              className="rounded-full border border-[color:var(--color-ink-200)] px-4 py-2 text-center text-xs font-semibold uppercase tracking-[0.2em] text-[color:var(--color-ink-800)] transition-colors hover:border-[color:var(--color-primary)] hover:text-[color:var(--color-primary)]"
              onClick={() => setOpen(false)}
            >
              Send News
            </NavLink>
