...
export default function InternationalNews() {
  ...
  const fetchInternationalNews = async () => {
    try {
      ...
      setInternationalNews(data.slice(0, 6)); // Limit to 6 articles
    } catch (error) {
    ...
    return (
      <section className="max-w-[1200px] mx-auto px-6 py-16">
        <div className="text-center py-12">
          <div className="inline-block w-8 h-8 border-4 border-[#0066cc] border-t-transparent rounded-full animate-spin mb-4" />
  ...
  if (internationalNews.length === 0) {
    return (
      <section className="max-w-[1200px] mx-auto px-6 py-16">
        <h2 className="text-[28px] font-playfair font-black text-[#0b1020] mb-8">International News</h2>
        <div className="text-center py-12">
          <p className="text-[16px] text-[#5a6073]">No international news available at the moment.</p>
        </div>
      </section>
    );
  }
  ...
  return (
    <section className="max-w-[1200px] mx-auto px-6 py-16">
      <div className="flex items-center justify-between mb-8">
        <h2 className="text-[28px] font-playfair font-black text-[#0b1020]">International News</h2>
        <Link 
          to="/international-news" 
          className="text-[14px] font-semibold text-[#0066cc] hover:text-[#004499] transition-colors"
        >
          View All →
        </Link>
      </div>
            ...
            <div className="bg-white rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow">
              <div className="relative h-48 overflow-hidden">
                ...
                <div className="absolute top-3 left-3">
                  <span className="inline-block px-2 py-1 text-white text-[10px] font-bold tracking-[0.1em] uppercase rounded bg-[#0066cc]">
                    International
                  </span>
                </div>
              </div>
              ...
              <div className="p-4">
                <h3 className="text-[16px] font-semibold text-[#0b1020] leading-[1.3] mb-2 group-hover:text-[#0066cc] transition-colors line-clamp-2">
                ...
                <div className="flex items-center gap-2 text-[12px] text-[#8b91a5]">
                  <span className="font-medium text-[#2c3348]">{formatAuthor(article.author)}</span>
                  <span>•</span>
                  <span>
                    {article.createdAt 
